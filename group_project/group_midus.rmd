...
```{r}
# calculate tertile and put categories in table
tertile1 <- quantile(full_data$B1SORIEN, 1/3)
tertile2 <- quantile(full_data$B1SORIEN, 2/3)

full_data$optimism_category <- case_when(
...
full_data$optimism_category <- factor(full_data$optimism_category, 
                                    levels = c("low", "moderate", "high"))
table(full_data$optimism_category)
```

```{r}
# age
age_row <- full_data %>% group_by(optimism_category) %>% summarize(age_mean = mean(age), age_sd = sd(age))
print(t(age_row))
```
```{r}
# highest education
print("Highest Education Gained")
education_numbers <- table(full_data$education_categorical, full_data$optimism_category)
column_percentage <- prop.table(education_numbers, margin = 2)
row_percentage <- prop.table(education_numbers, margin = 1)
print(column_percentage)
print(row_percentage)
print(chisq.test(education_numbers))
```
```{r}
# race
print("Race")
race_table <- table(full_data$race, full_data$optimism_category)
column_percentage <- prop.table(race_table, margin = 2) * 100
row_percentage <- prop.table(race_table, margin = 1) * 100
print(column_percentage)
print(row_percentage)
print(chisq.test(race_table))
```

```{r}
# gender
print("Gender")
gender_table <- table(full_data$sex, full_data$optimism_category)
column_percentage <- prop.table(gender_table, margin = 2) * 100
row_percentage <- prop.table(gender_table, margin = 1) * 100
print(column_percentage)
print(row_percentage)
...
chronic_table <- table(full_data$chronic_condition, full_data$optimism_category)
column_percentage <- prop.table(chronic_table, margin = 2) * 100
print(column_percentage)
row_percentage <- prop.table(chronic_table, margin = 1) * 100
print(row_percentage)
print(chisq.test(chronic_table))
```

```{r}
print("Income interval")
df.income_interval = data.frame(t(as.matrix(full_data %>% group_by(optimism_category) %>% summarize(mean(visit_interval), sd(visit_interval), mean(household_income), sd(household_income)))))

names(df.income_interval) <- df.income_interval[1,]
df.income_interval<-df.income_interval[-1,]
df.income_interval

chisq.test(full_data %>% select(race, optimism_category)%>% table())
```
```{r}
print("Blood pressure medication use")
cont_tab_bp <- table(full_data$blood_pressure_med, full_data$optimism_category)
row_percents_bp <- prop.table(cont_tab_bp, margin = 1)
col_percents_bp <- prop.table(cont_tab_bp, margin = 2)

formatted_table_bp <- rbind(
  "Yes (Column %)" = col_percents_bp[2, ],  
  "Yes (Row %)" = row_percents_bp[2, ],    
  "No (Column %)" = col_percents_bp[1, ],  
  "No (Row %)" = row_percents_bp[1, ]      
)

print(formatted_table_bp)


# chi-square test
chisq.test(cont_tab_bp)
...
negative_affect_summary <- full_data |> 
  ...
  summarize(
    mean = mean(negative_affect),
    sd = sd(negative_affect)) |> 
  mutate(mean_sd = paste0(round(mean, 2), " ± ", round(sd, 2))) |> 
  select(optimism_category, mean_sd) 

# Print the summary
print(negative_affect_summary)

# anova test
anova(lm(negative_affect ~ optimism_category, data = full_data))
